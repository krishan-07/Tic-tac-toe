...
function checkWin(row, column) {
    //for row
    if (gameBoard[row][0] === playerMove && gameBoard[row][1] === playerMove && gameBoard[row][2] === playerMove) { return true; }
    //for column
    if (gameBoard[0][column] === playerMove && gameBoard[1][column] === playerMove && gameBoard[2][column] === playerMove) { return true; }
    //for diagonal
    if (gameBoard[0][0] === playerMove && gameBoard[1][1] === playerMove && gameBoard[2][2] === playerMove || gameBoard[0][2] === playerMove && gameBoard[1][1] === playerMove && gameBoard[2][0] === playerMove) { return true; }
...
function updateScore() {
    if (xWin > 0)
        displayScoreX.innerHTML = xWin;
    else if(xWin === 0)
        displayScoreX.innerText = '-';

    if (oWin > 0)
        displayScoreO.innerHTML = oWin;
    else if(oWin === 0)
        displayScoreO.innerText = '-';
